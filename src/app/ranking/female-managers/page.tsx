...
export default function FemaleManagersPage() {
  const data = getFemaleManagerRanking(200);

  return (
    <div className="flex flex-col min-h-full bg-mesh">
      <RankingJsonLd
        listName="女性管理職比率ランキング（上場企業）"
        path="/ranking/female-managers"
        breadcrumbLabel="女性管理職比率"
        items={data}
      />
      <Header />
      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-8 sm:px-6 lg:px-8 space-y-6">
        <nav className="flex items-center gap-2 text-sm text-[var(--color-text-muted)]">
          ...
          <span>/</span>
          <span className="text-[var(--color-text-primary)]">女性管理職比率</span>
        </nav>
        ...
        <div>
          ...
          <p className="text-sm text-[var(--color-text-muted)]">
            有価証券報告書ベース・DEI指標トップ{data.length}社
          </p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-secondary)]">
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold text-[var(--color-text-muted)]">女性管理職比率</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-[var(--color-text-muted)] hidden md:table-cell">男女賃金格差</th>
                ...
                {data.map((row: any, i: number) => (
                  <tr key={row.code} className="border-b border-[var(--color-border)] hover:bg-[var(--color-surface-secondary)] transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-gradient">
                      {row.femaleManagerRate}%
                    </td>
                    <td className="px-4 py-3 text-right hidden md:table-cell">
                      {row.genderWageGapAll != null ? (
                        <span className="text-[var(--color-text-secondary)]">{row.genderWageGapAll}%</span>
                      ) : "-"}
                    </td>
        ...
        <div className="glass rounded-2xl p-4 text-xs text-[var(--color-text-muted)] leading-relaxed">
          ...
          2023年3月期以降に開示義務化されたDEI指標（女性活躍推進法・改正労働施策総合推進法）に基づくデータです。
          男女賃金格差は女性賃金÷男性賃金で算出。100%が完全均等。
        ...
        <RankingNav current="/ranking/female-managers" />
      </main>
      <Footer />
    </div>
  );
